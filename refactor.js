...
let code = fs.readFileSync('src/pages/SessionView.jsx', 'utf-8');
...
code = code.replace(
      ...
      showInfo(\\ is already in the call\);
    },
      ...
      showInfo(\\ participant(s) already in the call\);
    }
);
...
code = code.replace(
      ...
      resetRemoteMedia();
      showInfo(data.name ? \\ left the call\ : 'Participant left the call');
    });,
        const participantLeft = on('participant-left', (data) => {
      setRemotePeers(prev => {
        const next = { ...prev };
...
// Second replacement for handleJoinCallAudioOnly
code = code.replace(
          for (const producer of joinedData?.producers || []) {
        setRemoteParticipant({
          id: producer.participantId,
...
fs.writeFileSync('src/pages/SessionView.jsx', code, 'utf-8');
